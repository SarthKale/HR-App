...
class Configuration:
    ...
    It needs a server.cfg which consist of the port number in the JSON format.
    Example:
        {host: host_name(str), port: port_number(int)}
    ...
    def __new__(cls):
        ...
        The data of connection are read from a custom file provided by the user
        by the name of 'server.cfg' in which host and port data are provided.
        """
        if Configuration._obj is not None:
            return Configuration._obj
        if not os.path.isfile("network_client/server.cfg"):
            print("Configuration file missing, refer documentation")
            sys.exit()
        try:
            with open("network_client/server.cfg") as json_file:
    ...
    def _validate_values(self):
        ...
        """
        if Configuration._obj.host is None:
            Configuration._obj.exceptions["host"] = (
                'V', "host entry is missing in server.cfg, refer documentation")
        elif not isinstance(Configuration._obj.host, str):
            Configuration._obj.exceptions["host"] = (
                'T', f"Host is of type {type(Configuration._obj.host)}, should be of type {type('A')}")
        if Configuration._obj.port is None:
            ...
            Configuration._obj.exceptions["port"] = ('T', f"Port is of type {type(Configuration._obj.port)}, should be of type {type(10)}")
        elif Configuration._obj.port < 1024 or Configuration._obj.port > 49151:
            Configuration._obj.exceptions["port"] = ('V', f"Port Value is {Configuration._obj.port}, it should be within Range(1024,49151)")
